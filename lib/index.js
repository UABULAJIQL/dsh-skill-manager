...
import { filterSkillCandidates, normalizeSkillManagerSettings } from './policy.js'
...
class ManagedSkillProvider {
  ...
  async list(options) {
    ...
    for (const candidate of candidates) this.#candidateProviders.set(candidate, provider)
    const filtered = filterSkillCandidates(candidates, this.#disabled)
    return Array.isArray(observed) ? filtered : { ...observed, candidates: filtered }
  }
