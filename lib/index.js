...
/** Start durable settings and install one provider in each agent's nearest scope. */
export async function apply(ctx) {
  const store = new SkillManagerStore(ctx)
  await store.load()

  const attachProvider = agent => {
    let provider
    const unregisterProvider = agent.ctx.skills.registerProvider(control => {
      provider = new ManagedSkillProvider(agent.ctx, control, store.snapshot())
      return provider
    })
    ...
    agent.ctx.effect(() => () => {
      unwatch()
      unregisterProvider()
    }, 'skill-manager agent provider')
  }

  const stopAgentListener = ctx.on('agent/created', ({ agent }) => { attachProvider(agent) })
