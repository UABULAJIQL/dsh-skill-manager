window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const MANAGED_PRESET_ID = "skill-manager-code";
    ...
    const zh = {
      ...
      managedPresetRequired: "当前会话未使用 Code (Skill Manager)，Skill 开关不会作用于此会话。请新建会话或切换空白会话的 preset。",
      enabled: "启用",
      disabled: "禁用",
    ...
    const en = {
      ...
      managedPresetRequired: "The current session does not use Code (Skill Manager), so Skill switches do not affect it. Start a new session or switch a blank session's preset.",
      enabled: "Enabled",
      disabled: "Disabled",
    ...
    function SkillManagerSection(props) {
      ...
      const currentSession = sessionId === undefined ? undefined : sessionState.byId?.[sessionId];
      const managedSession = currentSession?.agentPreset === MANAGED_PRESET_ID;
      ...
      return h("div", { style: styles.section, ref: rootRef },
        ...
        sessionId !== undefined && currentSession !== undefined && !managedSession ? h("p", { style: styles.error, role: "status" }, t("managedPresetRequired")) : null,
        ...
        h("section", { style: styles.group },
          ...
          sessionId === undefined ? h("p", { style: styles.status }, t("noSession")) : catalogState.status === "loading" ? h("p", { style: styles.status }, t("loading")) : catalogState.status === "error" ? h("p", { style: styles.error }, t("loadFailed") + ": " + catalogState.error) : rows.length === 0 ? h("p", { style: styles.status }, t("empty")) : rows.map(row => {
            ...
            return h("div", { key: row.name, style: styles.row },
              ...
              h("input", { style: styles.checkbox, type: "checkbox", checked: row.enabled, disabled: !managedSession || !settingsState.writable || busy, "aria-label": row.name, onChange: () => toggleSkill(row.name) })
            );
          })
        )
      );
