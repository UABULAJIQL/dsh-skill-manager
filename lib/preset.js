...
/** Make managed sessions the default without changing an explicit non-code choice. */
export async function ensureManagedCodePresetDefault(ctx) {
  if (ctx.agentPresets.defaultId !== 'code') return
  await ctx.settings.mutate('agent-presets', [
    { op: 'set', path: ['default'], value: MANAGED_PRESET_ID },
  ])
}
